...
// +----------------------------------------------------------------------+
//
// $Id: UtilsPropertiesBLTI.php,v 1.1 2007/12/12 11:53:56 jmassaguer Exp $
...
class UtilsPropertiesBLTI {
  ...
  private   $properties = array(); // associative array of key => Properties_Section_Property_BLTI object references.
  ...
  public function setProperty($key, $value) {
    //Properties_Section_Property_BLTI::testKey($key);
    //Properties_Section_Property_BLTI::testValue($key);
    if (array_key_exists($key, $this->properties)) {
      $this->properties[$key]->setValue($value);
    }
    else {
      $p = new Properties_Section_Property_BLTI($key, $value);
		    ...
		    for ($i = 0; $i < count($lines); $i++) {     
		      ...
		      if (strlen($line) == 0) {
		        if ($i < count($lines) -1) { // don't store last blank.
		          $p = new Properties_Section_Blank_BLTI();
		      ...
		      elseif (preg_match('/^(#|!)(\\s*)(.*)$/', $line, $matches)) {
		        $p = new Properties_Section_Comment_BLTI($matches[3], $matches[1], $matches[2]);
		        array_push($sections, $p);
		      }
		      ...
		      // handle properties or multiline sections.
		        ...
		        // handle properties
		          ...
		          $p = new Properties_Section_Property_BLTI($line);
		          $key = $p->getKey();
		          if (array_key_exists($key, $properties)) {
		            // Property name already exists.
		            array_push($sections, new Properties_Section_Comment_BLTI("WARNING: The following previously encountered property was commented out:\n$line"));
		          }
		          else {
		            $properties[$key] = $p;
		            array_push($sections, $p);
  ...
  public function store($file, $header = null) {
    ...
    if (isset($header) && strlen($header)) {
      $comment = new Properties_Section_Comment_BLTI($header, false);
 ...
 /**
   ...
   * @return boolean
   */
  public function equals(UtilsPropertiesBLTI $other) {
  ...
  public function remove($key) {
    ...
    if (array_key_exists($key, $this->properties)) {
      ...
      for ($i = 0; $i < count($this->sections); $i++) {
        if (($this->sections[$i] instanceof Properties_Section_Property_BLTI) && ($this->sections[$i]->getKey() == $key)) {
          $splice_from = $splice_to = $i;
          if ($i > 0) {
            $previous_section_class = get_class($this->sections[$i - 1]);
            if ($previous_section_class != 'Properties_Section_Property_BLTI') {
              for ($j = $i - 1; $j >= 0; $j--) {
 ...
 */
interface Properties_ISection_BLTI {
...
/****************************** End of interface Properties_ISection_BLTI ******************************/
 ...
 */
class Properties_Section_Blank_BLTI implements Properties_ISection_BLTI {
...
/****************************** End of class Properties_Section_Blank_BLTI ******************************/
 ...
 */
class Properties_Section_Comment_BLTI implements Properties_ISection_BLTI {
...
/****************************** End of class Properties_Section_Comment_BLTI ******************************/
 ...
 */
class Properties_Section_Property_BLTI implements Properties_ISection_BLTI {
  ...
  public function __construct() {
    ...
    case 1:
      $line = func_get_arg(0);
      if (isset($line)) {
        $line = ltrim($line, UtilsPropertiesBLTI::WHITE_SPACE_CHARS);
      }
...
/****************************** End of class Properties_Section_Property_BLTI ******************************/
